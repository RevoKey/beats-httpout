...
func (out *httpOutput) init(cfg *common.Config) error {
	...
	waitRetry := time.Duration(1) * time.Second
	maxWaitRetry := time.Duration(60) * time.Second
...
func makeClientFactory(tls *transport.TLSConfig, config *httpConfig, out *httpOutput) func(string) (mode.ProtocolClient, error) {
	return func(host string) (mode.ProtocolClient, error) {
		hostURL, err := getURL(config.Protocol, config.Path, host)
		if err != nil {
...
func (out *httpOutput) PublishEvent(sig op.Signaler, opts outputs.Options, data outputs.Data) error {
	return out.mode.PublishEvent(sig, opts, data)
}
